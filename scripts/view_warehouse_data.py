...
class WarehouseViewer:
    ...
    def _view_services(self):
        ...
        if services:
            ...
            if self.detailed:
                ...
                print(f"  Category: {getattr(sample, 'category', 'N/A')}")
    ...
    def _view_procedures(self):
        ...
        if procedures and self.detailed:
            print(
                "\n{:<5} {:<40} {:<15} {:<15}".format("ID", "Title", "Type", "Service")
            )
            print("-" * 80)

            for proc in procedures[:10]:  # Show first 10
                service_name = (
                    self.db.query(Service).get(proc.service_id).name
                    if proc.service_id
                    else "N/A"
                )
                print(
    ...
    def _view_documents(self):
        ...
        if self.detailed and documents:
            ...
            print(
                f"  Raw Content (first 200 chars): {sample.raw_content[:200] if sample.raw_content else 'N/A'}"
            )
    ...
    def _view_content_chunks(self):
        ...
        # Group by chunk type
        by_category = (
            self.db.query(ContentChunk.chunk_type, func.count(ContentChunk.chunk_id))
            .group_by(ContentChunk.chunk_type)
            .all()
        )

        if by_category:
            print("\n📊 Chunks by Type:")
            print("{:<20} {:>10}".format("Type", "Count"))
            print("-" * 32)
    ...
    def _view_raw_content(self):
        ...
        by_source = (
            self.db.query(RawContent.source_type, func.count(RawContent.content_id))
            .group_by(RawContent.source_type)
            .all()
        )
        ...
        by_status = (
            self.db.query(
                RawContent.processing_status, func.count(RawContent.content_id)
            )
            .group_by(RawContent.processing_status)
            .all()
        )
        ...
        if self.detailed and raw_contents:
            ...
            print(
                f"  Content (first 200 chars): {sample.content[:200] if sample.content else 'N/A'}"
            )
...
def main():
    ...
    parser.add_argument(
        "--export", action="store_true", help="Export data to JSON files"
    )
    parser.add_argument(
        "--tables",
        type=str,
        default="",
        help="Comma-separated tables to show (services,procedures,documents,faqs,content_chunks,raw_content,stats)",
    )
    ...
    if "services" in selected:
        viewer._view_services()
    if "procedures" in selected:
        viewer._view_procedures()
    if "documents" in selected:
        viewer._view_documents()
    if "faqs" in selected:
        viewer._view_faqs()
    if "content_chunks" in selected:
        viewer._view_content_chunks()
    if "raw_content" in selected:
        viewer._view_raw_content()
    if "stats" in selected or not selected:
        viewer._view_database_stats()
