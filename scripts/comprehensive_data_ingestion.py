...
class ComprehensiveDataIngestion:
    ...
    def _ensure_base_services(self):
        ...
        for service_data in BASE_SERVICES:
            ...
            existing = (
                ...
                .filter(service_category_col == service_data["category"])
                .first()
            )
    ...
    def _ingest_pdf_documents(self):
        ...
        for category_dir, category in CATEGORY_MAP.items():
            ...
            if category:
                ...
                service = (
                    self.db.query(Service)
                    .filter(service_category_col == category)
                    .first()
                )
    ...
    def _create_chunks(
        self, text: str, service: Service = None, chunk_size: int = 500
    ) -> list:
        ...
        for chunk_text in self._create_chunks(text, service):
            self.db.add(
                ContentChunk(
                    ...
                    chunk_type=getattr(service, "category", "general"),
                )
            )
