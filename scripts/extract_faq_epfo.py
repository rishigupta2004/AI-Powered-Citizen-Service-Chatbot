#!/usr/bin/env python
"""Extract FAQs from EPFO scraper."""
import sys
sys.path.insert(0, '.')
...
def main():
    print('=== EXTRACTING EPFO FAQs ===')
    db = SessionLocal()
    embedding_model = get_transformer()
    
    try:
        # Get EPFO service
        service = db.query(Service).filter(Service.name.ilike('%epfo%')).first()
        service_id = service.service_id if service else 4
        ...
        db.commit()
        print(f'✓ Added: {added} new EPFO FAQs')
