#!/usr/bin/env python
"""Extract FAQs from Aadhaar scraper."""
import sys
sys.path.insert(0, '.')
...
def main():
    print('=== EXTRACTING AADHAAR FAQs ===')
    db = SessionLocal()
    embedding_model = get_transformer()
    
    try:
        # Get Aadhaar service
        service = db.query(Service).filter(Service.name.ilike('%aadhaar%')).first()
        service_id = service.service_id if service else 2
        
        scraper = AadhaarScraper()
        ...
        db.commit()
        print(f'✓ Added: {added} new Aadhaar FAQs')
