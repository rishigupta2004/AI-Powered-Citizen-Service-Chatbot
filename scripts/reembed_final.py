...
# ── Chunks ────────────────────────────────────────────────────────────────────
cur.execute("SELECT chunk_id, chunk_text FROM content_chunks WHERE chunk_text IS NOT NULL")
chunks = cur.fetchall()
...
for i, (fid, q, a) in enumerate(faqs):
    if q:
        ...
        if qvec:
            cur.execute("UPDATE faqs SET question_embedding = %s::vector WHERE faq_id = %s",
                       (str(qvec), fid))
    if a:
        avec = eng.embed_text(a[:512], is_query=False)
        if avec:
            cur.execute("UPDATE faqs SET answer_embedding = %s::vector WHERE faq_id = %s",
                       (str(avec), fid))
    if (i+1) % 20 == 0:
        conn.commit()
