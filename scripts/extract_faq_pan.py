...
def main():
    print('=== EXTRACTING PAN FAQs ===')
    db = SessionLocal()
    embedding_model = get_transformer()
    
    try:
        # Get PAN service
        service = db.query(Service).filter(Service.name.ilike('%pan%')).first()
        service_id = service.service_id if service else 3
        ...
        db.commit()
        print(f'✓ Added: {added} new PAN FAQs')
