...
class Service(Base):
    ...
    procedures = relationship(
        "Procedure", back_populates="service", cascade="all, delete-orphan"
    )
    documents = relationship(
        "Document", back_populates="service", cascade="all, delete-orphan"
    )
...
class Procedure(Base):
    ...
    uuid = Column(UUID(as_uuid=True), default=uuid.uuid4, unique=True)
    service_id = Column(
        Integer, ForeignKey("services.service_id", ondelete="CASCADE"), nullable=False
    )
...
class Document(Base):
    ...
    procedure_id = Column(
        Integer, ForeignKey("procedures.procedure_id", ondelete="CASCADE")
    )
...
class FAQ(Base):
    ...
    language = Column(String(10), default="en")
    question_embedding = Column(Vector(EMBEDDING_DIM))
    answer_embedding = Column(Vector(EMBEDDING_DIM))
    created_at = Column(TIMESTAMP(timezone=True), server_default=func.now())
...
class ContentChunk(Base):
    ...
    service_id = Column(
        Integer, ForeignKey("services.service_id", ondelete="CASCADE"), nullable=True
    )
...
try:
    ...
    Index(
        "idx_documents_tsv",
        text(
            "to_tsvector('english', coalesce(name,'') || ' ' || coalesce(description,'') || ' ' || coalesce(raw_content,''))"
        ),
