...
def run():
    ...
    print(f"{GRAY}  Target: all 6 auth endpoints reachable, DB tables exist, JWT working{RESET}")
    ...
    optional_vars = ["DIGILOCKER_CLIENT_ID", "DIGILOCKER_CLIENT_SECRET", "DIGILOCKER_REDIRECT_URI"]
    env_ok = 0
    for var in required_vars:
        val = os.getenv(var, "")
        passed = bool(val)
        check(f"{var}", passed, "set" if passed else "MISSING — add to .env and flyctl secrets")
        if passed: env_ok += 1
    ...
    try:
        ...
        required_tables = ["users", "user_sessions", "content_chunks", "services", "faqs"]
        for t in required_tables:
            check(f"table '{t}' exists", t in tables, "found" if t in tables else "MISSING — run python init_db.py")
    ...
    for method, path, expected_codes, note in endpoints:
        ...
        try:
            ...
            check(f"{method} {path}", passed, f"HTTP {r.status_code} — {note} — {latency}ms")
        except Exception as e:
    ...
    try:
        ...
        if secret:
            ...
            check("JWT encode/decode", decoded["sub"] == "test_user_123", f"token length={len(token)}")
        else:
    ...
    try:
        r = requests.post(f"{BASE_URL}/api/auth/otp/send", json={"phone": "invalid"}, timeout=10)
        check("send-otp rejects invalid phone", r.status_code in [400, 422], f"HTTP {r.status_code}")
    except Exception as e:
        fail(f"send-otp test failed: {e}")

    # Test verify-otp with wrong OTP (should return 400/401 not 500)
    try:
        r = requests.post(f"{BASE_URL}/api/auth/otp/verify", json={"phone": "+919999999999", "otp": "000000"}, timeout=10)
        check("verify-otp rejects wrong OTP", r.status_code in [400, 401, 422], f"HTTP {r.status_code}")
    except Exception as e:
    ...
    try:
        ...
        check("/auth/me blocks unauthenticated", r.status_code in [401, 403], f"HTTP {r.status_code}")
    except Exception as e:
        fail(f"Protected route check failed: {e}")

    # Test with fake JWT (should reject)
    try:
        r = requests.get(f"{BASE_URL}/api/auth/me", headers={"Authorization": "Bearer fakejwt123"}, timeout=10)
        check("/auth/me rejects invalid JWT", r.status_code in [401, 403, 422], f"HTTP {r.status_code}")
    except Exception as e:
